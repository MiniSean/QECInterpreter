...
from qce_interp.interface_definitions.intrf_state_classification import ShotsClassifierContainer
...
class LookupTableDecoder(ISyndromeDecoder, metaclass=ABCMeta):
    ...
    def eigenvalue_syndrome_lookup(self) -> Dict[tuple, tuple]:
        ...
        return {
            tuple(ShotsClassifierContainer.binary_to_eigenvalue(np.asarray(key))): tuple(
                ShotsClassifierContainer.binary_to_eigenvalue(np.asarray(value)))
    ...
    def get_binary_syndrome_corrections(self, cycle_stabilizer_count: int) -> NDArray[np.int_]:
        ...
        return ShotsClassifierContainer.eigenvalue_to_binary(result)
    ...
    def get_binary_projected_corrected(self, cycle_stabilizer_count: int) -> NDArray[np.int_]:
        ...
        # Pre-process
        eigenvalue_projected_classification = ShotsClassifierContainer.binary_to_eigenvalue(binary_projected_classification)
        eigenvalue_syndrome_correction = ShotsClassifierContainer.binary_to_eigenvalue(binary_syndrome_correction)
        # (N, 1, D)
        ...
        # Post-process
        binary_projected_corrected: NDArray[np.int_] = ShotsClassifierContainer.eigenvalue_to_binary(eigenvalue_projected_corrected)
